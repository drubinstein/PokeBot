...
function inventory.indexOf(name)
    ...
    for i = 0, 19 do
        ...
        if (memory.raw(iidx) == searchID) then return i end
    end
    return -1
end
...
function inventory.count(name)
    ...
    if (index ~= -1) then return memory.raw(ITEM_BASE + index * 2 + 1) end
    return 0
end

function inventory.contains(...)
    for i, name in ipairs(arg) do
        if (inventory.count(name) > 0) then return name end
    end
end
...
function inventory.teach(item, poke, replaceIdx, altPoke)
    ...
    elseif (main == 64 or main == 96 or main == 192) then
        ...
        elseif (column == 15) then
            ...
            local idx = 0
            if (poke) then idx = pokemon.indexOf(poke, altPoke) end
...
function inventory.isFull() return memory.raw(0x1145) > 0 end
...
function inventory.use(item, poke, midfight)
    ...
    if (main == 144) then
        if (memory.value("battle", "menu") == 95) then
            ...
            local idx = 0
            if (poke) then idx = pokemon.indexOf(poke) end
            menu.select(idx, true)
        end
    elseif (main == 128 or main == 60) then
        ...
        elseif (column == 14) then
            ...
            local index = 0
            if (poke) then index = pokemon.indexOf(poke) end
