...
local function getAddress(index) return 0x116B + index * 0x2C end
...
local function index(index, offset)
    ...
    if (double) then value = value + memory.raw(address + 1) end
    return value
end
pokemon.index = index
...
local function indexOf(...)
    for ni, name in ipairs(arg) do
        ...
        for i = 0, 5 do
            local atIdx = index(i)
            if (atIdx == pid) then return i end
        end
    end
    return -1
end
...
function pokemon.battleMove(name)
    ...
    for i = 1, 4 do if (mid == memory.raw(0x101B + i)) then return i end end
end
...
function pokemon.moveIndex(move, pokemon)
    ...
    for i = 1, 4 do if (mid == memory.raw(address + i)) then return i end end
end

function pokemon.info(name, offset) return index(indexOf(name), offset) end

function pokemon.getID(name) return pokeIDs[name] end

function pokemon.getName(id)
    for name, pid in pairs(pokeIDs) do if (pid == id) then return name end end
end

function pokemon.inParty(...)
    for i, name in ipairs(arg) do
        if (indexOf(name) ~= -1) then return name end
    end
end
...
function pokemon.forMove(move)
    ...
    for i = 0, 5 do
        ...
        for j = 8, 11 do
            if (memory.raw(address + j) == moveID) then return i end
        end
    end
    return -1
end

function pokemon.hasMove(move) return pokemon.forMove(move) ~= -1 end

function pokemon.updateParty()
    local partySize = memory.value("player", "party_size")
    if (partySize ~= previousPartySize) then
        local poke = pokemon.inParty("oddish", "paras", "spearow", "pidgey",
                                     "nidoran", "squirtle")
        if (poke) then
            bridge.caught(poke)
...
function pokemon.isOpponent(...)
    ...
    for i, name in ipairs(arg) do
        if (oid == pokeIDs[name]) then return name end
    end
end
...
function pokemon.isEvolving() return memory.value("menu", "pokemon") == 144 end

function pokemon.getExp() return memory.raw(0x117A) * 256 + memory.raw(0x117B) end
