...
local damageMultiplier =
    { -- http://bulbapedia.bulbagarden.net/wiki/Type_chart#Generation_I
        normal = {
            normal = 1.0,
            fighting = 1.0,
            flying = 1.0,
...
local function isDisabled(mid) return mid == memory.value("battle", "disabled") end
combat.isDisabled = isDisabled

local function calcDamage(move, attacker, defender, rng)
    if (move.fixed) then return move.fixed, move.fixed end
    if (move.power == 0 or isDisabled(move.id)) then return 0, 0 end
    if (move.power > 9000) then
        local oid = defender.id
        if (oid ~= 14 and oid ~= 147 and oid ~= 171 and
            (oidd ~= 151 or memory.value("game", "map") == 120)) then -- ???
            if (memory.value("battle", "x_accuracy") == 1 and defender.speed <
                attacker.speed) then return 9001, 9001 end
        end
        return 0, 0
    end
    if (move.name == "Thrash" and combat.disableThrash) then return 0, 0 end
    ...
    local damage = floor(floor(floor(2 * attacker.level / 5 + 2) *
                                   math.max(1, attFactor) * move.power /
                                   math.max(1, defFactor)) / 50) + 2
    ...
    if (defender.type1 == defender.type2) then typeEffect2 = 1 end
    damage = floor(damage * typeEffect1 * typeEffect2)
    if (rng) then return damage, damage end
...
local function getOpponentType(ty)
    ...
    if ty ~= 0 then
        ...
        if not t1 then return memory.value("battle", "opponent_type2") end
    end
    ...
    for idx, move in ipairs(attacker.moves) do
        if (not move.pp or move.pp > 0) then
            ...
            if (maxDmg) then
                ...
                if (ours) then
                    local replaces
                    if (not ret or minTurns < bestMinTurns or maxTurns <
                        bestTurns) then
                        replaces = true
                    elseif (maxTurns == bestTurns and move.name == "Thrash") then
                        replaces = defender.hp ==
                    ...
                    elseif (enablePP) then
                        if (maxTurns < 2 or maxTurns == bestMaxTurns) then
                            if (ret.name == "Earthquake" and
                                (move.name == "Ice-Beam" or move.name ==
                                    "Thunderbolt")) then
                                replaces = true
...
local function isSleeping() return memory.raw(0x116F) > 1 end
combat.isSleeping = isSleeping

-- Combat AI functions

function combat.factorPP(enabled) enablePP = enabled end

function combat.reset() enablePP = false end
...
function combat.inKillRange(draw)
    ...
    if (not turnsToKill or not enemyAttack) then return false end
    if (draw) then
        gui.text(0, 21, ours.speed .. " " .. enemy.speed)
        gui.text(0, 28, turnsToDie .. " " .. ours.hp .. " | " .. turnsToKill ..
                     " " .. enemy.hp)
    end
    local hpReq = enemyAttack.damage
    local isConfused = memory.value("battle", "confused") > 0
    if (isConfused) then hpReq = hpReq + math.floor(ours.hp * 0.2) end
    if (ours.hp < hpReq) then
        ...
        if (outspeed or isConfused or turnsToKill > 1 or ours.speed <=
            enemy.speed or isSleeping()) then return ours, hpReq end
    end
end
...
local function getBattlePokemon()
    ...
    for idx = 1, 4 do
        ...
        if (move) then move.midx = idx end
    end
    return ours, enemy
end
...
function combat.bestMove()
    local ours, enemy = getBattlePokemon()
    if (enemy) then return getBestMove(ours, enemy) end
end
