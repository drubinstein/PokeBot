...
local timeRequirements = {
    mankey = function()
        local timeLimit = 33
        if (pokemon.inParty("paras")) then timeLimit = timeLimit + 0.75 end
        return timeLimit
    end,
    ...
    vermilion = function() return 44.25 end,

    trash = function()
        local timeLimit = 47
        if (nidoSpecial > 44) then timeLimit = timeLimit + 0.25 end
        if (nidoAttack > 53) then timeLimit = timeLimit + 0.25 end
    ...
    safari_carbos = function() return 70.5 end,

    e4center = function() return 102 end,

    blue = function() return 108.2 end
}
...
local function hardReset(message, extra)
    ...
    if (strategies.seed) then message = message .. " | " .. strategies.seed end
    print(message)
    -- bridge.chat(message)
    -- client.reboot_core()
...
local function reset(reason, extra)
    ...
    if (time) then resetString = resetString .. " after " .. time end
    if (areaName) then resetString = " " .. resetString .. " at " .. areaName end
    local separator
...
local function overMinute(min) return utils.igt() > min * 60 end

local function resetTime(timeLimit, reason, once)
    if (overMinute(timeLimit)) then
        reason = "Took too long to " .. reason
        if (RESET_FOR_TIME) then return reset(reason) end
        if (once) then print(reason .. " " .. paint.elapsedTime()) end
    end
end

local function getTimeRequirement(name) return timeRequirements[name]() end
...
local function opponentDamaged(factor)
    if (not factor) then factor = 1 end
    return memory.double("battle", "opponent_hp") * factor <
               memory.double("battle", "opponent_max_hp")
end

local function redHP() return math.ceil(pokemon.index(0, "max_hp") * 0.2) end
...
local function dodgeUp(npc, sx, sy, dodge, offset)
    if (not battle.handleWild()) then return false end
    local px, py = player.position()
    if (py < sy - 1) then return true end
    local wx, wy = px, py
    if (py < sy) then
        wy = py - 1
...
local function dodgeH(options)
    local left = 1
    if (options.left) then left = -1 end
    local px, py = player.position()
...
local function completedMenuFor(data)
    ...
    if (count == 0 or count + (data.amount or 1) <= tries) then return true end
    return false
end
...
local function useItem(data)
    ...
    if (tries == 0) then
        ...
        if (tries == 0) then
            if (closeMenuFor(data)) then return true end
            return false
        end
    end
...
local function completedSkillFor(data)
    if (data.map) then
        if (data.map ~= memory.value("game", "map")) then return true end
    elseif (data.x or data.y) then
        local px, py = player.position()
        if (data.x == px or data.y == py) then return true end
    elseif (data.done) then
        if (memory.raw(data.done) > (data.val or 0)) then return true end
...
local function nidoranDSum(disabled)
    ...
    if (not disabled and tries ~= 0) then
        if (tries[tries.idx] == 0) then
            ...
            if (tries.idx > 3) then tries = 0 end
            return nidoranDSum()
        end
...
strategyFunctions = {
    ...
    reportFrames = function()
        ...
        if (repels > 0) then print("S " .. repels) end
    ...
    tweetVictoryRoad = function()
        local elt = paint.elapsedTime()
        bridge.tweet("Entering Victory Road at " .. elt ..
                         " on our way to the Elite Four! http://www.twitch.tv/thepokebot")
        return true
    end,
    ...
    interact = function(data)
        if (battle.handleWild()) then
            ...
            if (textbox.isActive()) then
                if (tries > 0) then return true end
                tries = tries - 1
                input.cancel()
    ...
    potion = function(data)
        ...
        if (curr_hp == 0) then return false end
        local toHP = data.hp
    ...
    teach = function(data)
        if (data.full and not inventory.isFull()) then return true end
        local itemName
        if (data.item) then
            itemName = data.item
        else
        ...
        if (pokemon.hasMove(data.move)) then
            local main = memory.value("menu", "main")
            if (main == 128) then
                if (data.chain) then return true end
            elseif (main < 3) then
                return true
            end
            input.press("B")
    ...
    skill = function(data)
        if (completedSkillFor(data)) then
            if (not textbox.isActive()) then return true end
            input.press("B")
    ...
    fly = function(data)
        if (memory.value("game", "map") == data.map) then return true end
        local cities = {
            pallet = {62, "Up"},
            viridian = {63, "Up"},
            lavender = {66, "Down"},
    ...
    bicycle = function()
        if (memory.raw(0x1700) == 1) then
            if (textbox.handle()) then return true end
        else
            return useItem({item = "bicycle"})
        end
    end,

    fightXAccuracy = function() return prepare("x_accuracy") end,
    ...
    waitToPause = function()
        local main = memory.value("menu", "main")
        if (main == 128) then
            if (canProgress) then return true end
        elseif (battle.isActive()) then
            canProgress = false
            battle.automate()
    ...
    squirtleIChooseYou = function()
        ...
        if (player.face("Up")) then textbox.name("A") end
    end,
    ...
    fightBulbasaur = function()
        if (tries < 9000 and pokemon.index(0, "level") == 6) then
            if (tries > 200) then
                ...
                if (squirtleAtt < 11 and squirtleScl < 12) then
                    return reset(
                               "Bad Squirtle - " .. squirtleAtt .. " attack, " ..
                                   squirtleScl .. " special")
                end
                tries = 9001
            else
        ...
        if (battle.isActive() and memory.double("battle", "opponent_hp") > 0 and
            resetTime(2.15, "kill Bulbasaur")) then return true end
        return buffTo("tail_whip", 6)
    end,

    dodgePalletBoy = function() return dodgeUp(0x0223, 14, 14, 15, 7) end,
    ...
    catchNidoran = function()
        if (not control.canCatch()) then return true end
        ...
        if (battle.isActive()) then
            ...
            if (isNidoran and memory.value("battle", "opponent_level") > 2) then
                if (initialize()) then bridge.pollForName() end
            end
            tries = nil
            ...
            if (resetTime(timeLimit, resetMessage)) then return true end
    ...
    dodgeViridianOldMan = function() return dodgeUp(0x0273, 18, 6, 17, 9) end,

    grabAntidote = function()
        local px, py = player.position()
        if (py < 11) then return true end
    ...
    fightWeedle = function()
        if (battle.isTrainer()) then
            ...
            if (squirtleOut and memory.value("battle", "our_status") > 0 and
                not inventory.contains("antidote")) then
            ...
            if (sidx == -1) then return buffTo("tail_whip", 5) end
            ...
            elseif (squirtleOut) then
                ...
                local forced
                if (peck and peck.damage and peck.damage + 1 >=
                    memory.double("battle", "opponent_hp")) then
                    forced = "growl"
                end
                battle.fight(forced)
            end
    ...
    equipForBrock = function(data)
        if (initialize()) then
            ...
            if (data.anti) then
                ...
                if (not poisoned) then return true end
                if (not inventory.contains("antidote")) then
                    return reset("Poisoned, but we skipped the antidote")
                end
                if (inventory.contains("potion") and
                    pokemon.info("squirtle", "hp") > 8) then
                    return true
                end
            end
        end
        ...
        if (nidoranIndex == 0) then
            if (menu.close()) then return true end
        elseif (menu.pause()) then
            local column = menu.getCol()
            if (pokemon.info("squirtle", "status") > 0) then
                inventory.use("antidote", "squirtle")
            elseif (inventory.contains("potion") and
                pokemon.info("squirtle", "hp") < 15) then
    ...
    fightBrock = function()
        ...
        if (squirtleHP == 0) then return resetDeath() end
        if (battle.isActive()) then
            ...
            elseif (turnsToDie and turnsToDie < 2 and
                inventory.contains("potion")) then
                ...
                elseif (bideTurns > 0) then
                    ...
                    if (turnsToKill) then
                        local forced
                        if (turnsToDie < 2 or turnsToKill < 2 or tempDir -
                            bideTurns > 1) then
                ...
                if (tries < 9000) then
                    ...
                    if (pokemon.index(nidx, "level") == 8) then
                        ...
                        if (tries > 300) then
                            local statDiff =
                                (16 - att) + (15 - spd) + (13 - scl)
                            if (def < 12) then
                                statDiff = statDiff + 1
                            end
                            local resets =
                                att < 15 or spd < 14 or scl < 12 or statDiff > 3
                            if (not resets and att == 15 and spd == 14) then
                                resets = true
                            end
                            local nStatus =
                                "Att: " .. att .. ", Def: " .. def ..
                                    ", Speed: " .. spd .. ", Special: " .. scl
                            if (resets) then
                            ...
                            nStatus = "Beat Brock with a" .. superlative ..
                                          " Nidoran" .. exclaim .. " " ..
                                          nStatus
                            bridge.chat(nStatus)
                        else
    ...
    pewterMart = function()
        return shop.transaction {
            buy = {
                {name = "potion", index = 1, amount = 7},
                {name = "escape_rope", index = 2}
            }
        }
    end,
    ...
    potionBeforeCocoons = function()
        if (yolo or nidoSpeed > 14) then return true end
        return strategyFunctions.potion({hp = 6})
    end,

    swapHornAttack = function()
        if (pokemon.battleMove("horn_attack") == 1) then return true end
        battle.swapMove(1, 3)
    end,
    ...
    fightMetapod = function()
        if (battle.isActive()) then
            canProgress = true
            if (memory.double("battle", "opponent_hp") > 0 and
                pokemon.isOpponent("metapod")) then return true end
            battle.automate()
        elseif (canProgress) then
            return true
        else
    ...
    catchFlierBackup = function()
        if (initialize()) then strategies.canDie = true end
        if (not control.canCatch()) then return true end
        ...
        if (battle.isActive()) then
            ...
            local px, py = player.position()
            if (caught) then
                if (px > 33) then return true end
                local startY = 9
                if (px > 28) then startY = py end
            ...
            if (birdPath) then walk.custom(birdPath) end
        end
    end,
    ...
    startMtMoon = function()
        ...
        if (skipHiker) then control.mtMoonExp() end
        return true
    end,
    ...
    evolveNidorino = function()
        ...
        elseif (tries > 3600) then
            ...
            if (canProgress) then tries = tries + 1 end
            input.press("A")
        end
    end,
    ...
    teachWaterGun = function()
        if (battle.handleWild()) then
            if (not pokemon.inParty("nidorino")) then
                ...
                print("")
                print("")
                return reset("Did not evolve to Nidorino",
                             pokemon.info("nidoran", "level"))
            end
            return strategyFunctions.teach(
                       {move = "water_gun", replace = "tackle"})
        end
    end,

    fightHiker = function()
        if (skipHiker) then return true end
    ...
    evolveNidoking = function()
        if (battle.handleWild()) then
            if (not inventory.contains("moon_stone")) then
                if (initialize()) then bridge.caught("nidoking") end
                if (menu.close()) then return true end
    ...
    helix = function()
        if (battle.handleWild()) then
            if (inventory.contains("helix_fossil")) then return true end
            player.interact("Up")
        end
    end,
    ...
    reportMtMoon = function()
        ...
        if (strategies.moonEncounters) then
            ...
            if (pokemon.inParty("paras")) then
                parasCatch = "paras"
                if (goodEncounters) then conjunction = "and" end
                parasStatus = "we found a Paras!"
            else
                parasCatch = "no_paras"
                if (not goodEncounters) then conjunction = "and" end
                parasStatus = "we didn't find a Paras :("
            end
            bridge.caught(parasCatch)
            bridge.chat(strategies.moonEncounters .. " Moon encounters, " ..
                            conjunction .. " " .. parasStatus)
        ...
        if (not skipHiker) then timeLimit = timeLimit + 0.25 end
        if (pokemon.inParty("paras")) then timeLimit = timeLimit + 1.0 end
    ...
    rivalSandAttack = function(data)
        if (battle.isActive()) then
                    ...
                    if (turns == 1) then
                        ...
                        if (not sacrifice) then
                            if (yolo) then
                                ...
                                temp = pokemon.inParty("oddish", "paras",
                                                       "squirtle")
                            end
                            if (temp and pokemon.info(temp, "hp") > 0) then
                                sacrifice = temp
            ...
            elseif (opponent == "raticate") then
                combat.disableThrash = opponentDamaged() or
                                           (not yolo and pokemon.index(0, "hp") <
                                               32) -- RISK
            elseif (opponent == "ivysaur") then
                if (not yolo and damaged(5) and
                    inventory.contains("super_potion")) then
    ...
    teachThrash = function()
        if (initialize()) then
            if (pokemon.hasMove("thrash") or pokemon.info("nidoking", "level") <
                21) then return true end
        end
        if (strategyFunctions.teach({
            move = "thrash",
            item = "rare_candy",
            replace = "leer"
        })) then
    ...
    redbarMankey = function()
        if (not setYolo("mankey")) then return true end
        local curr_hp, red_hp = pokemon.index(0, "hp"), redHP()
        if (curr_hp <= red_hp) then return true end
        if (initialize()) then
            if (pokemon.info("nidoking", "level") < 21 or
                inventory.count("potion") < 3) then -- RISK
                return true
            end
        ...
        if (battle.isActive()) then
            ...
            if (enemyTurns) then
                if (enemyTurns < 2) then return true end
    ...
    potionBeforeRocket = function()
        ...
        if (yolo) then minAttack = minAttack - 1 end
        if (nidoAttack >= minAttack) then return true end
    ...
    catchOddish = function()
        ...
        if (battleValue > 0) then
            if (battleValue == 2) then
                ...
                if (tries == 0 and py == 31) then tries = 1 end
                if (not control.shouldCatch()) then battle.run() end
            end
        elseif (tries == 1 and py == 31) then
            ...
            elseif (px == 12) then
                ...
                local path = {
                    {15, 19}, {15, 25}, {15, 25}, {15, 27}, {14, 27}, {14, 30},
                    {12, 30}
                }
                walk.custom(path)
            end
    ...
    vermilionMart = function()
        if (initialize()) then setYolo("vermilion") end
        local buyArray, sellArray
        if (not inventory.contains("pokeball") or (not yolo and nidoAttack < 53)) then
            sellArray = {
                {name = "pokeball"}, {name = "antidote"}, {name = "tm34"},
                {name = "nugget"}
            }
            buyArray = {
                {name = "super_potion", index = 1, amount = 3},
                {name = "paralyze_heal", index = 4, amount = 2},
                {name = "repel", index = 5, amount = 3}
            }
        else
            sellArray = {
                {name = "antidote"}, {name = "tm34"}, {name = "nugget"}
            }
            buyArray = {
                {name = "super_potion", index = 1, amount = 3},
                {name = "repel", index = 5, amount = 3}
        ...
        return shop.transaction {sell = sellArray, buy = buyArray}
    end,
    ...
    trashcans = function()
        ...
        if (textbox.isActive()) then
            if (not canProgress) then
                if (progress < 2) then tries = tries + 1 end
                canProgress = true
            end
            input.cancel()
        else
            if (progress == 3) then
                ...
                if (px == 4 and py == 6) then
                    ...
                    if (resetTime(timeLimit,
                                  "complete Trashcans (" .. tries .. " tries)")) then
                        return true
                    end
                    setYolo("trash")
                    local prefix
                    ...
                    bridge.chat(prefix .. ", " .. tries .. " try Trashcans" ..
                                    suffix, paint.elapsedTime())
                    return true
                end
                local completePath = {
                    Down = {{2, 11}, {8, 7}},
            ...
            elseif (progress == 2) then
                ...
                local trashPath = {
                    {2, 11}, {"Left"}, {2, 11}, {2, 12}, {4, 12}, {4, 11},
                    {"Right"}, {4, 11}, {4, 9}, {"Left"}, {4, 9}, {4, 7},
                    {"Right"}, {4, 7}, {4, 6}, {2, 6}, {2, 7}, {"Left"}, {2, 7},
                    {2, 6}, {4, 6}, {4, 8}, {7, 8}, {"Down"}, {7, 8}, {9, 8},
                    {"Up"}, {9, 8}, {8, 8}, {8, 11}, {"Right"}, {8, 11}
                }
    ...
    dodgeBicycleGirlRight = function()
        return dodgeH {npc = 0x0222, sx = 4, sy = 5, dodge = 4, offset = -2}
    end,
    ...
    swapBicycle = function()
        ...
        if (bicycleIdx < 3) then return true end
        local main = memory.value("menu", "main")
        if (main == 128) then
            ...
            if (pokemon.isOpponent("cubone")) then
                ...
                if (enemyTurns) then
                    ...
                    if (forced and initialize()) then
                        bridge.chat(
                            "Using Thunderbolt to attempt to redbar off Cubone")
                    end
                end
            end
            battle.automate(forced)
    ...
    shopBuffs = function()
        local minSpecial = 45
        if (yolo) then minSpecial = minSpecial - 1 end
        ...
        return shop.transaction {
            direction = "Up",
            buy = {
                {name = "x_accuracy", index = 0, amount = 10},
                {name = "x_speed", index = 5, amount = 4},
                {name = "x_special", index = 6, amount = xspecAmt}
            }
        }
    end,
    ...
    shopVending = function()
        return shop.vend {
            direction = "Up",
            buy = {
                {name = "fresh_water", index = 0},
                {name = "soda_pop", index = 1}
            }
        }
    end,
    ...
    swapRepels = function()
        ...
        if (repelIdx < 3) then return true end
        local main = memory.value("menu", "main")
        if (main == 128) then
            if (menu.getCol() ~= 5) then
                menu.select(2, true)
    ...
    lavenderRival = function()
        if (battle.isActive()) then
            ...
            if (nidoSpecial > 44) then -- RISK
                local __, enemyTurns = combat.enemyAttack()
                if (enemyTurns and enemyTurns < 2 and
                    pokemon.isOpponent("pidgeotto", "gyarados")) then
                    battle.automate()
                    return false
                end
            end
    ...
    digFight = function()
        if (battle.isActive()) then
            ...
            elseif (pokemon.info("nidoking", "hp") == 0 and
                pokemon.index(backupIndex, "hp") == 0 and
                pokemon.isDeployed("paras", "squirtle")) then
                return resetDeath()
            else
                battle.fight("dig")
            end
    ...
    safariCarbos = function()
        if (initialize()) then setYolo("safari_carbos") end
        local minSpeed = 50
        if (yolo) then minSpeed = minSpeed - 1 end
        if (nidoSpeed >= minSpeed) then return true end
        if (inventory.contains("carbos")) then
            if (walk.step(20, 20)) then return true end
        else
            local px, py = player.position()
            if (px < 21) then
                walk.step(21, py)
    ...
    fightSilphMachoke = function()
        if (battle.isActive()) then
            canProgress = true
            if (nidoSpecial > 44) then return prepare("x_accuracy") end
    ...
    silphCarbos = function()
        if (nidoSpeed > 50) then return true end
        return strategyFunctions.interact({dir = "Left"})
    end,
        ...
        if (battle.isActive()) then
            ...
            if (prepare("x_accuracy", "x_speed")) then
                ...
                if (pokemon.isOpponent("pidgeot")) then
                    if (riskGiovanni or nidoSpecial < 45 or
                        pokemon.info("nidoking", "hp") > 85) then
                        forced = "thunderbolt"
                    end
    ...
    fightHypno = function()
        if (battle.isActive()) then
            ...
            if (pokemon.isOpponent("hypno")) then
                if (pokemon.info("nidoking", "hp") >
                    combat.healthFor("KogaWeezing") * 0.9) then
    ...
    earthquakeElixer = function(data)
        ...
        if (initialize()) then
            if (areaName) then print("EQ Elixer: " .. areaName) end
        end
        return useItem({
            item = "elixer",
            poke = "nidoking",
    ...
    checkGiovanni = function()
        if (initialize()) then
            ...
            if (earthquakePP > 1) then
                if (riskGiovanni and earthquakePP > 2 and
                    battle.pp("horn_drill") > 4 and
                    (yolo or pokemon.info("nidoking", "hp") >
                        combat.healthFor("GiovanniRhyhorn") * 0.925)) then -- RISK
                    bridge.chat(
                        "Using risky strats on Giovanni to skip the extra Max Ether...")
                else
                    riskGiovanni = false
                end
                return true
    ...
    fightGiovanniMachoke = function(data) return prepare("x_special") end,
        ...
        if (tempDir and completedMenuFor(data)) then
            ...
            if (not tempDir) then
                if (data.max) then
                    ...
                    maxEtherSkip = nidoAttack > 53 and battle.pp("earthquake") >
                                       0 and battle.pp("horn_drill") > 3
                    if (maxEtherSkip) then return true end
                    bridge.chat(
                        "Grabbing the Max Ether to skip the Elite 4 Center")
                end
                tempDir = inventory.contains("ether", "max_ether")
                if (not tempDir) then return true end
    ...
    pickMaxEther = function()
        if (not canProgress) then
            ...
            if (memory.value("player", "moving") == 0) then
                ...
                tries = not tries
                if (tries) then input.press("Right", 1) end
            end
            return false
        end
        if (inventory.contains("max_ether")) then return true end
        player.interact("Right")
    end,
    ...
    healBeforeLorelei = function()
        if (initialize()) then
            local canPotion
            if (inventory.contains("potion") and
                hasHealthFor("LoreleiDewgong", 20)) then
                canPotion = true
            elseif (inventory.contains("super_potion") and
                hasHealthFor("LoreleiDewgong", 50)) then
                canPotion = true
            end
            if (not canPotion) then return true end
            bridge.chat("Healing before Lorelei to skip the Elite 4 Center...")
        end
        return strategyFunctions.potion({
            hp = combat.healthFor("LoreleiDewgong")
        })
    end,
    ...
    depositPokemon = function()
        ...
        if (memory.value("player", "party_size") == toSize) then
            ...
            if (not textbox.isActive()) then
                ...
                if (memory.value("battle", "menu") ~= 95 and
                    (pc == 2 or pc == 4)) then
    ...
    lorelei = function()
        if (battle.isActive()) then
            ...
            if (pokemon.isOpponent("dewgong")) then
                local sacrifice = pokemon.inParty("pidgey", "spearow",
                                                  "squirtle", "paras", "oddish")
                if (sacrifice and pokemon.info(sacrifice, "hp") > 0) then
                    battle.swap(sacrifice)
                    return false
                end
            end
            if (prepare("x_accuracy")) then battle.automate() end
        elseif (canProgress) then
            return true
        else
            textbox.handle()
    ...
    bruno = function()
        if (battle.isActive()) then
            ...
            if (prepare("x_accuracy")) then battle.automate(forced) end
        elseif (canProgress) then
            return true
        else
            textbox.handle()
    ...
    agatha = function()
        if (battle.isActive()) then
            ...
            if (pokemon.isOpponent("gengar")) then
                ...
                if (not yolo and currentHP <= 56 and
                    not isPrepared("x_accuracy", "x_speed")) then
                    local toPotion = inventory.contains("full_restore",
                                                        "super_potion")
                    if (toPotion) then
    ...
    lance = function()
        if (tries == 0) then
            tries = {
                {"x_special", inventory.count("x_special")},
                {"x_speed", inventory.count("x_speed"), 89}
            }
        end
        return prepare()
    end,
    ...
    prepareForBlue = function()
        if (initialize()) then setYolo("blue") end
    ...
    blue = function()
        if (battle.isActive()) then
            canProgress = true
            if (memory.value("battle", "turns") > 0 and
                not isPrepared("x_accuracy", "x_speed")) then
                local toPotion = inventory.contains("full_restore",
                                                    "super_potion")
            ...
            if (not tempDir) then
                if (nidoSpecial > 45 and pokemon.index(0, "speed") > 52 and
                    inventory.contains("x_special")) then
                    tempDir = "x_special"
                else
                    tempDir = "x_speed"
                end
        ...
        if (canProgress) then
            ...
            if (tries == 0) then
                ...
                if (strategies.seed) then
                    print(memory.value("game", "frames") ..
                              " frames, with seed " .. strategies.seed)
                    print(
...
function strategies.execute(data)
    if (strategyFunctions[data.s](data)) then
        ...
        initialized = false
        tempDir = nil
        if (resetting) then return nil end
        return true
    end
    return false
end

function strategies.init(midGame) if (midGame) then combat.factorPP(true) end end
