...
local PAINT_ON = false -- Displays contextual information while the bot runs
...
local function resetAll()
    ...
    if (CUSTOM_SEED) then
        ...
        print("RUNNING WITH A FIXED SEED (" .. strategies.seed ..
                  "), every run will play out identically!")
    else
...
if (RESET_FOR_TIME and hasAlreadyStartedPlaying) then
    ...
    print(
        "Disabling time-limit resets as the game is already running. Please reset the emulator and restart the script if you'd like to go for a fast time.")
end
...
while true do
    ...
    if (not input.update()) then
        if (not utils.ingame()) then
                ...
                if (battleState == 1) then
                    if (not inBattle) then
                        ...
                        if (strategies.moonEncounters) then
                            strategies.moonEncounters =
                                strategies.moonEncounters + 1
                        end
                        inBattle = true
                    end
                end
            ...
            if (currentHP == 0 and not strategies.canDie and pokemon.index(0) >
                0) then
    ...
    if (PAINT_ON) then paint.draw(currentMap) end
