...
function battle.isActive() return memory.value("game", "battle") > 0 end

function battle.isTrainer()
    local battleType = memory.value("game", "battle")
    if (battleType == 2) then return true end
...
function battle.handleWild()
    if (memory.value("game", "battle") ~= 1) then return true end
    battle.run()
end

function battle.fight(move, isNumber, skipBuffs)
    if (move) then
        if (not isNumber) then move = pokemon.battleMove(move) end
        attack(move)
    else
