...
function paint.draw(currentMap)
    ...
    if (nidx ~= -1) then
        ...
        gui.text(0, 0, "Nido Stats:")
        gui.text(0, 4, att .. " " .. def .. " " .. spd .. " " .. scl)
    end
    local enc = " encounter"
    if (encounters ~= 1) then enc = enc .. "s" end
...
function paint.wildEncounters(count) encounters = count end

function paint.reset() encounters = 0 end
