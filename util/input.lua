...
local function sendButton(button, ab)
    ...
    -- local inputTable = {[button] = true}
    bridge.send(lunajson.encode({["method"] = "button", ["payload"] = button}))
    if (_debug) then print(button .. " " .. remainingFrames) end
    if (ab) then button = "AB" end
    setForFrame = button
end
...
function input.press(button, frames)
    ...
    if (frames == nil or frames > 0) then
        if (button == currentButton) then return end
        if (not frames) then frames = 1 end
...
function input.setDebug(enabled) _debug = enabled end

function input.test(fn, completes)
    while (true) do
        if (not input.update()) then
            if (fn() and completes) then break end
        end
        -- emu.frameadvance()
        bridge.send(lunajson.encode({["method"] = "frame_advance"}))
    end
    if (completes) then print(completes .. " complete!") end
end
